...
def _build_excel_from_donnees(donnees, titre):
    ...
    for _, e in donnees.get('ecoles', {}).items():
        vals = [
            ...
            float(e['depenses'].get('montant_total', 0) or 0),
    ...
    # Feuille 3: remises par catégorie
    ws3 = wb.create_sheet('Remises par catégorie')
    headers3 = ['École', 'Catégorie', 'Montant']
    ws3.append(headers3)
    ...
    for i, w in enumerate([26, 24, 16], start=1):
...
def collecter_donnees_journalieres(date_rapport, user=None):
    ...
    for ecole in ecoles_qs:
        ...
        donnees_ecole['remises_par_categorie'] = [
            {
                'motif': row['motif'] or '',
                'label': motif_labels.get(row['motif'], row['motif'] or 'Non précisé'),
                'total': row['total'] or Decimal('0'),
            }
            for row in remises_categorie_qs
        ]
        ...
        if eleves_concernes_ids:
            ...
            paiements_cumules = Paiement.objects.filter(
                eleve_id__in=list(eleves_concernes_ids),
                annee_scolaire=annee_scolaire,
                date_paiement__lte=date_rapport,
                statut='VALIDE',
            ).filter(filtre_types_scolarite()).values('eleve_id').annotate(total=Sum('montant'))
            paye_par_eleve = {row['eleve_id']: row['total'] or Decimal('0') for row in paiements_cumules}
            remises_cumulees = PaiementRemise.objects.filter(
                ...
                paiement__date_paiement__lte=date_rapport,
                paiement__statut='VALIDE',
            ).filter(
                filtre_types_scolarite('paiement__type_paiement')
            ).values('paiement__eleve_id').annotate(total=Sum('montant_remise'))
            remises_par_eleve = {
                row['paiement__eleve_id']: row['total'] or Decimal('0')
                for row in remises_cumulees
            }
            for row in qs_ech:
                ...
                paye = min(du, paye_par_eleve.get(row['eleve_id'], Decimal('0')))
                remises_eleve = remises_par_eleve.get(row['eleve_id'], Decimal('0'))
                solde = max(Decimal('0'), du - paye - remises_eleve)
...
def generer_pdf_journalier(donnees, date_rapport):
    ...
    for ecole_id, donnees_ecole in donnees['ecoles'].items():
        ...
        if donnees_ecole.get('remises_par_categorie'):
            ...
            story.append(Spacer(1, 6))
            remise_data = [['Catégorie', 'Montant']]
            for r in donnees_ecole['remises_par_categorie']:
                remise_data.append([
                    r['label'],
                    f"{r['total']:,} GNF".replace(',', ' '),
                ])
            remise_table = Table(remise_data, colWidths=[200, 120])
