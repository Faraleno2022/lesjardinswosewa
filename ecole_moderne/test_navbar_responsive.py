...
class NavbarResponsiveTests(SimpleTestCase):
    ...
    def test_navigation_se_replie_avant_1400_pixels(self):
        self.assertEqual(self.response.status_code, 200)
        self.assertContains(
            self.response,
            'navbar navbar-expand-xxl navbar-dark bg-primary fixed-top site-navbar',
        )
        self.assertContains(self.response, '@media (max-width: 1399.98px)')
